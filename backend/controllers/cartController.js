...
export const addToCart = async (req, res) => {
  ...
  try {
    ...
    // ✅ Ép kiểu discount từ chuỗi sang number
    const discountStr = product.discount || "0%";
    const discountNum = parseInt(discountStr.replace('%', '')) || 0;
    ...
    if (index >= 0) {
      ...
      cart.items.push({ productId, name, price, image, quantity, discount: discountNum });
    }
...
export const syncCart = async (req, res) => {
  ...
  try {
    ...
    // ✅ Chuyển discount về number khi sync
    console.log('Sync items nhận được:', items);
    ...
    console.error('Lỗi khi đồng bộ giỏ hàng:', err.message, err.stack);
