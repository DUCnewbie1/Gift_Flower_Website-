...
export const searchProducts = async (req, res) => {
  try {
    ...
    let products = await Product.find({
      ...
      .limit(20) // Tăng giới hạn để có nhiều sản phẩm hơn trước khi lọc theo khu vực
      .select("name category price image stockByStore"); // Thêm stockByStore

    // Nếu có district, lọc theo khu vực
    if (district) {
      ...
      // Chỉ giữ lại sản phẩm có tồn kho ở khu vực này
      products = products.filter(product => {
        return product.stockByStore && product.stockByStore.some(stock => 
          storeIds.includes(stock.storeId.toString()) && stock.quantity > 0
        );
      });
    }
