...
import regionalPriceRoutes from './routes/regionalPriceRoutes.js';
...
app.post('/api/create-momo-payment', async (req, res) => {
  ...
  if (isNaN(amountInt) || amountInt <= 0) return res.status(400).json({ error: 'Số tiền không hợp lệ' });

  const redirectUrl = defaultReturnUrl;
  const orderInfo   = `Thanh toan don hang ${orderId}`;
  const extra       = { userId, phone: customerInfo?.phone, email: customerInfo?.email };
  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response) {
      res.status(error.response.status).json({ error: 'Không thể tạo thanh toán MoMo', details: error.response.data });
    } else {
...
app.post('/api/save-order', async (req, res) => {
  ...
  try {
    ...
    if (existingOrder) {
      return res.status(200).json({ message: 'Đơn hàng đã tồn tại', cartCleared: true });
    }
    ...
    res.status(201).json({ message: 'Đơn hàng đã được lưu', cartCleared: true });
  } catch (error) {
    console.error('Lỗi khi lưu đơn hàng:', error);
    res.status(500).json({ error: 'Không thể lưu đơn hàng', details: error.message });
  }
});

app.post('/api/create-order', async (req, res) => {
  const { orderId, cartItems, totalAmount, customerInfo, paymentMethod, status } = req.body;
  try {
    const order = new Order({ orderId, amount: totalAmount, customerInfo, cartItems, paymentMethod, status });
    await order.save();
...
app.post('/api/update-product-quantity', async (req, res) => {
  ...
  try {
    for (const item of cartItems) {
      ...
      if (!product) return res.status(404).json({ error: `Sản phẩm ${item.name} không tồn tại` });
      
      // Sử dụng stockByStore thay vì stock
      const totalStock = product.stockByStore?.reduce((sum, s) => sum + s.quantity, 0) || 0;
      if (totalStock < item.quantity) return res.status(400).json({ error: `Số lượng tồn kho không đủ cho sản phẩm ${item.name}` });
      
      // Cập nhật stockByStore thay vì stock
      // Cần logic phân bổ số lượng giảm vào các store
      // Đây là một cách đơn giản: giảm từ store đầu tiên
      let remainingQuantity = item.quantity;
      for (let i = 0; i < product.stockByStore.length && remainingQuantity > 0; i++) {
        const store = product.stockByStore[i];
        const reduceAmount = Math.min(store.quantity, remainingQuantity);
        store.quantity -= reduceAmount;
...
app.post('/momo-webhook', async (req, res) => {
  ...
  const extraData = req.body.extraData ? JSON.parse(Buffer.from(req.body.extraData, 'base64').toString()) : {};
  ...
  if (calculatedSignature !== signature) return res.status(400).send('Invalid signature');

  if (resultCode === 0) {
    await Order.create({ orderId, transactionId: transId, amount, paymentMethod: 'TRANSFER', status: 'SUCCESS', userId });
    if (userId) await Cart.deleteMany({ userId });
  } else {
    await Order.create({ orderId, transactionId: transId, amount, paymentMethod: 'TRANSFER', status: 'FAILED', userId });
  }
...
app.use('/api/regional-prices', regionalPriceRoutes);
