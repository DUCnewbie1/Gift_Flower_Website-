    ...
    <>
        ...
        <div className="flex gap-4">
            ...
            <div className="w-full h-64 bg-pink-100 rounded-lg relative overflow-hidden">
                ...
                >
                    <div className="text-center bg-pink-100/70 px-6 py-3 rounded-lg">
                    <h2 className="text-3xl font-bold text-pink-600">{slide.title}</h2>
                    <p className="text-xl font-bold text-pink-600">{slide.subtitle}</p>
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                {[1, 2, 3, 4, 5].map((idx) => (
                <div key={idx} className="flex gap-2 pb-3 border-b">
                    <img 
                    src={`/product-${idx}.jpg`}
                    alt={`New product ${idx}`}
                    ...
                    <div className="text-xs font-medium line-clamp-2">
                        {idx % 2 === 0 ? 'Bộ quà tặng Luvgift Sweet' : 'Bộ chăm sóc da mặt cây lô hội'}
                    </div>
                    <div className="text-pink-600 font-bold text-sm mt-1">
                        {idx % 2 === 0 ? '215.000₫' : '165.000₫'}
                    </div>
                    </div>
                </div>
                ))}
                {/* Wrap these two divs in a fragment or a parent div */}
                <>
