...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToCart = (product: Omit<CartItem, 'quantity'>, quantity = 1) => {
    ...
    setCartItems(prev => {
      ...
      if (existing) {
        // Kiểm tra nếu tổng số lượng vượt quá tồn kho khu vực
        if (product.regionalStock !== undefined && existing.quantity + quantity > product.regionalStock) {
          toast.error(`Không thể thêm. Tổng số lượng sẽ vượt quá tồn kho khu vực (${product.regionalStock})`);
          return prev;
        }
      ...
      axios
        .post('http://localhost:5000/api/cart/sync', {
          ...
          items: cartItems.map(item => ({
            productId: item._id, // <-- send as productId
            name: item.name,
            price: item.price,
            image: item.image,
