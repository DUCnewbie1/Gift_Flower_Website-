...
export default function FavoritesPage() {
  ...
  const getRegionalPrice = (id: string, defaultPrice: number) => {
    return regionalProducts[id]?.price ?? defaultPrice;
  };
  
  const isProductAvailable = (id: string) => {
    return regionalProducts[id]?.regionalStock > 0;
  };
  ...
  const handleAddToCart = (product: {
    ...
    const isAvailable = isProductAvailable(product._id);
    if (!isAvailable) {
      toast.error("Sản phẩm tạm hết hàng trong khu vực bạn chọn");
      return;
    }
  
    const price = getRegionalPrice(product._id, product.price);
  ...
  useEffect(() => {
    const fetchRegionalProducts = async () => {
      if (selectedDistrict) {
        try {
          const res = await axios.get(`http://localhost:5000/api/products/by-region?district=${encodeURIComponent(selectedDistrict)}`);
          const map = res.data.reduce((acc: Record<string, any>, product: any) => {
            acc[product._id] = product;
            return acc;
          }, {});
          setRegionalProducts(map);
        } catch (err) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {favoriteItems.map((item) => (
              <div key={item._id} className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden relative group">
                ...
                <a href={`/detail/${item._id}`} className="block">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col">
                      <span className="text-pink-600 font-bold">
                        {Math.round(item.price * (1 - parseInt(item.discount ?? "0") / 100)).toLocaleString()}₫
                      </span>
                    ...
                    <button
                      ...
                      disabled={!isProductAvailable(item._id)}
                      title={isProductAvailable(item._id) ? "Thêm vào giỏ hàng" : "Tạm hết hàng"}
                    >
