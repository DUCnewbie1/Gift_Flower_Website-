...
const SearchResults: React.FC = () => {
  ...
  const [isNavigating, setIsNavigating] = useState<string | null>(null); // Thêm trạng thái tải khi click
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (data.length > 0) {
          ...
          const categories = [...new Set(data.map((product: Product) => product.category))] as string[];
