...
export default function ProductListPage() {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.data.length > 0) {
          const prices = response.data.map((product: Product) => product.price);
          ...
          const categories = [...new Set(response.data.map((product: Product) => product.category))] as string[];
  ...
  const handleAddToCart = (product: Product) => {
    addToCart({
      _id: product._id,  // Use _id directly instead of converting
      name: product.name,
      price: product.price,
