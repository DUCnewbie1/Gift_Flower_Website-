...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      setIsLoading(true);
      try {
        // Nếu có selectedDistrict, chỉ lấy từ API by-region
        if (selectedDistrict) {
          ...
          const regionalProducts: ProductType[] = regionalRes.data;
          const regionalProduct = regionalProducts.find((p: ProductType) => p._id === id);

          if (!regionalProduct) {
            // Fallback: lấy sản phẩm theo ID nhưng set regionalStock = 0
            const fallbackRes = await axios.get(`http://localhost:5000/api/products/${id}`);
            const fallbackProduct = fallbackRes.data;
            setProduct({
              ...fallbackProduct,
              regionalStock: 0 // set tồn kho khu vực về 0
            });
            setSelectedImage(fallbackProduct.image || "/placeholder.png");
            fetchProductReviews(fallbackProduct._id);
          } else {
            setProduct(regionalProduct);
            setSelectedImage(regionalProduct.image || "/placeholder.png");
            fetchProductReviews(regionalProduct._id);
          }
          
        } else {
          // Nếu không có khu vực, fallback lấy sản phẩm chung
        ...
        setError("Không tìm thấy sản phẩm hoặc không khả dụng ở khu vực này.");
        toast.error("Không tìm thấy sản phẩm trong khu vực đã chọn");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  }, [id, selectedDistrict, fetchProductReviews]);

  useEffect(() => {
    const fetchRelatedProducts = async () => {
      if (!product?.category) return;
      try {
        const res = await axios.get(`http://localhost:5000/api/products?category=${encodeURIComponent(product.category)}`);
        let baseProducts = res.data.filter((p: ProductType) => p._id !== product._id);
  
        if (selectedDistrict) {
          const regionalRes = await axios.get(`http://localhost:5000/api/products/by-region?district=${encodeURIComponent(selectedDistrict)}`);
          const regionalMap = new Map<string, ProductType>(
            (regionalRes.data as ProductType[]).map(p => [p._id, p])
          );
              
          baseProducts = baseProducts
            .filter((p: ProductType) => regionalMap.has(p._id) && (regionalMap.get(p._id)?.regionalStock ?? 0) > 0)
            .map((p: ProductType) => ({
              ...p,
        ...
        setRelatedProducts(baseProducts);
      } catch (error) {
  ...
  const handleAddToCart = (product: ProductType) => {
    addToCart({
      ...
      originalPrice: product.discount !== "0%" 
        ...
        : undefined,
      discount: parseInt(product.discount?.replace('%', '') || '0'),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            >
              ...
              <div className="flex flex-col gap-1 mb-6">
                <span className="text-3xl font-bold text-pink-600">
                  {product.originalPrice
                    ...
                    : formatCurrency(Math.round(product.price * (1 - parseInt(product.discount || "0") / 100)))}
                </span>
                {product.regionalStock === 0 && (
                  <div className="text-red-500 text-sm font-medium mt-1">Tạm hết hàng trong khu vực</div>
                )}
              ...
              <div className="grid grid-cols-2 gap-4 mt-auto">
                <button
                  ...
                  disabled={product.regionalStock === 0}
                  className={`py-3 px-6 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-lg font-medium 
                    ${product.regionalStock === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:from-pink-600 hover:to-rose-600'} 
                ...
                <button
                  ...
                  disabled={product.regionalStock === 0}
                  className={`py-3 px-6 border-2 border-pink-500 text-pink-500 rounded-lg font-medium 
                    ${product.regionalStock === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-pink-50'} 
        ...
        >
          ...
          {relatedProducts.length === 0 ? (
            <p className="text-gray-500 text-center py-8">Hiện chưa có sản phẩm liên quan</p>
          ) : (
            <Swiper
              spaceBetween={16}
              slidesPerView={2}
            ...
            >
              {relatedProducts.slice(0, 10).map((item) => (
                ...
                <div className="group relative rounded-lg overflow-hidden shadow-sm border border-gray-200 hover:shadow-md transition-shadow bg-white p-2 min-h-[360px] flex flex-col">
                  <Link to={`/detail/${item._id}`}>
                    <div className="w-full aspect-square bg-gray-100 mb-2 overflow-hidden rounded-md">
                      <img
                  ...
                  <div className="absolute top-2 right-2 flex flex-col gap-2 z-10">
                    {/* Yêu thích */}
                    <button
                      className={`w-8 h-8 rounded-full shadow-md flex items-center justify-center transition-colors ${isInFavorites(item._id) ? "bg-pink-500 text-white" : "bg-white text-pink-500 border border-pink-300"}`}
                      onClick={(e) => {
                        e.preventDefault();
                      ...
                      onClick={(e) => {
                        ...
                        addToCart({
                          ...
                          originalPrice: item.discount !== "0%" 
                            ? Math.round(item.price * (1 - parseInt(item.discount?.replace('%', '') || '0') / 100))
                            : undefined,
                          discount: parseInt(item.discount?.replace('%', '') || '0'),
                          image: item.image || "/placeholder.png",
                          regionalStock: item.regionalStock,
                        });
                      }}
                  ...
                  <div className="p-4 flex flex-col justify-between flex-1">
                    <Link to={`/detail/${item._id}`} className="text-xs font-medium line-clamp-2 mb-2 hover:text-pink-600 transition-colors">
                      {item.name}
                    </Link>
              
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-pink-600 font-bold text-sm">{Math.round(item.price * (1 - parseInt(item.discount ?? '0') / 100)).toLocaleString()}₫</span>
                        {item.discount && item.discount !== "0%" && (
                          <div className="flex items-center mt-1">
                            <span className="text-gray-400 line-through text-xs">{item.price.toLocaleString()}₫</span>
